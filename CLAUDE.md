...
- **DMI observations** — `dmi.js` fetches real wind measurements from the nearest DMI automatic weather station (within ~50 km). Only activates for DK/GL/FO. Uses the keyless open-data endpoint `https://opendataapi.dmi.dk/v2/metObs` — no registration needed. Observations (10-min intervals, past 48 h) fetch `wind_speed`, `wind_gust_always_10min`, and `wind_dir` in parallel; all three are merged by timestamp into `{t, wind, gust, dir}` entries stored on `window.DMI_OBS`. `_dmiFindStation` now returns `{ nearest, all }` — every active station in the bbox is stored in `window.DMI_STATIONS` immediately after the station-list fetch (one API call, no extra calls per station). Status shown in the header as "📡 Obs: StationName · N km". Non-fatal: silently skipped outside Denmark or when the API is unavailable.
- **DMI station arrows** — `_dmiLatestWindObs` fetches the last 3 h of `wind_speed` + `wind_dir` for each non-nearest station using a **single combined request** (`parameterId=wind_speed,wind_dir` — OGC Features API comma-joined, NOT percent-encoded). Each station object gets a `.latest = { wind, gust, dir, time }` snapshot; the nearest station also gets `.obsHistory` pre-populated from the 48 h data. `_refreshDmiMarker` renders ALL in-bbox stations as `ws-wrap` arrow icons (identical style to the `wind-speeds.json` markers) when `.latest.wind` is available; stations without obs fall back to a teal dot. `window.dmiLoadStationHistory(station)` fetches and caches 24 h obs on demand for popup use.
- **DMI rate-limit protection** — Non-nearest station fetches run in **batches of `DMI_CONCURRENCY` (2)** with a `DMI_DELAY_MS_DEFAULT` (250 ms) pause between batches. This prevents bursting the API with too many concurrent requests. The delay is suppressed in unit tests via `window.DMI_DELAY_MS = 0`. Each non-nearest station uses one combined `_dmiObsMultiParam` call instead of two separate calls, halving the request count. 429 errors on individual stations are caught inside `_dmiLatestWindObs` and silently return null; other batch failures log a warning but never abort the overall load. Markers refresh progressively after each batch rather than once at the very end.
- **DMI popup history** — clicking any DMI station marker opens a popup showing station name, distance, latest wind/gust/direction, and a scrollable 24 h observation table (Time / Wind / Gust / Dir). History is lazy-loaded on first open via `window.dmiLoadStationHistory`; the nearest station's history is pre-cached from the 48 h fetch so its popup is instant. History is cached on `station.obsHistory` and not re-fetched on subsequent opens.
