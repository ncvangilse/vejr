...
- **Radar map wind stations** — `radar.js` fetches live observation snapshots from the DMI NinJo API (`https://www.dmi.dk/NinJo2DmiDk/ninjo2dmidk?cmd=obj&south=54.1&north=57.9&west=5.5&east=17.9`). The response is a dict keyed by station ID (e.g. `"06019"`); only stations with `WindSpeed10m != null` get a marker. Arrow direction from `WindDirection10m`, gust from `WindGustLast10Min`. NinJo doesn't send CORS headers, so `fetchNinjoStations()` tries the direct URL first (works on dmi.dk-hosted deploys), then falls back to `allorigins.win` / `corsproxy.io`. Refreshed every 10 minutes. Clicking a marker lazy-loads 24 h obs history via `window.dmiLoadStationHistory` (uses the DMI obs API — NinJo station IDs match DMI station IDs). `window.refreshDmiMarker` is kept as a no-op for backward compatibility with `dmi.js` callbacks.
