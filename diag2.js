// Diagnostic: trace buildClosedCoastRings for Vordingborg fixture
const fs = require('fs');
...
p('bbox: s='+bbox.s.toFixed(4)+' n='+bbox.n.toFixed(4)+' w='+bbox.w.toFixed(4)+' e='+bbox.e.toFixed(4));
...
// --- geometry helpers (copied from shore.js) ---
const EPS = 1e-9;
...
const TOL=1e-5;
const CORNERS=[{lat:bbox.n,lon:bbox.e},{lat:bbox.s,lon:bbox.e},{lat:bbox.s,lon:bbox.w},{lat:bbox.n,lon:bbox.w}];
function snapToBbox(pt,box){
  ...
  if(m===dN)return{lat:box.n,lon:cO(pt.lon)};
  if(m===dS)return{lat:box.s,lon:cO(pt.lon)};
  if(m===dE)return{lat:cL(pt.lat),lon:box.e};
  return{lat:cL(pt.lat),lon:box.w};
}
function cpos(pt,box){
  const {s,n,w,e}=box;
  if(Math.abs(pt.lon-e)<TOL)return(n-pt.lat)/(n-s);
  if(Math.abs(pt.lat-s)<TOL)return 1+(e-pt.lon)/(e-w);
  if(Math.abs(pt.lon-w)<TOL)return 2+(pt.lat-s)/(n-s);
  return 3+(pt.lon-w)/(e-w);
}
function cwPath(from, to, box){
  const {s,n,w,e}=box;
  const corners=[{lat:n,lon:e},{lat:s,lon:e},{lat:s,lon:w},{lat:n,lon:w}];
  const fS=snapToBbox(from,box), tS=snapToBbox(to,box);
  let fP=cpos(fS,box), oTP=cpos(tS,box), tP=oTP;
  if(tP<=fP){
    if(Math.floor(fP)===Math.floor(oTP))return[tS];
    tP+=4;
  }
  const cors=[];
...
// --- for each open chain, show entry/exit and closure ---
chains.forEach((c,i)=>{
  ...
  if(closed){
    p('chain['+i+'] n='+c.length+' CLOSED head=('+h.lat.toFixed(4)+','+h.lon.toFixed(4)+')');
    return;
  }
  ...
  p('chain['+i+'] n='+c.length+
    ' head=('+h.lat.toFixed(4)+','+h.lon.toFixed(4)+')['+hTag+']'+
    ' tail=('+t.lat.toFixed(4)+','+t.lon.toFixed(4)+')['+tTag+']');
  p('  entry=('+to.lat.toFixed(4)+','+to.lon.toFixed(4)+')'+
    ' exit=('+from.lat.toFixed(4)+','+from.lon.toFixed(4)+')');
  p('  closure('+closure.length+'): '+closure.map(p=>'('+p.lat.toFixed(4)+','+p.lon.toFixed(4)+')').join(' → '));
});

fs.writeFileSync('diag2_out.txt', out.join('\n')+'\n');
